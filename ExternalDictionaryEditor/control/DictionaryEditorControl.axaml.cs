...
using ReactiveUI;
using ExternalDictionaryEditor.viewmodel;
using ExternalDictionaryEditor.views;
using OpenUtau.Core;
using OpenUtau.Core.Ustx;
using Serilog;

namespace ExternalDictionaryEditor.control {
    public partial class DictionaryEditorControl : UserControl {
        ...
        private void EnsureLocalStringsLoaded() {
            if (Application.Current != null) {
                try {
                    var uri = new Uri("avares://ExternalDictionaryEditor/Strings.axaml");
                    var resourceInclude = new ResourceInclude(uri) {
                        Source = uri
                    };
                    Application.Current.Resources.MergedDictionaries.Add(resourceInclude);
                } catch {
                    // Ignore if already added or running stand-alone
                }
            }
        }
        ...
        private async System.Threading.Tasks.Task DoShowParseErrorAsync(ReactiveUI.IInteractionContext<ExternalDictionaryEditor.viewmodel.DictionaryErrorWindowViewModel, bool> interaction) {
